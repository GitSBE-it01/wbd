...
const mtnc = await api_access('get','mtnc_data_sb3','');
...
//----------------------------------------------
  if(event.target.getAttribute('data-method') === 'jig_submit') {
    ...
    if(ins_mstr.length >0) {
      const result = await api_access('insert', 'master_data_sb3', ins_mstr) ;
      if(result.includes('fail')) {
        alert('proses data error');
        return;
      } else {
        const result = await api_access('insert', 'master_log_sb3', ins_mstr) ;
      }
    }
    ...
    if(ins_loc.length >0) {
      const result = await api_access('insert', 'loc_data_sb3', ins_loc) ;
      if(result.includes('fail')) {
        alert('proses data error');
        return;
      } else {
        const result = await api_access('insert', 'loc_log_sb3', ins_loc) ;
      }
    }
    ...
    if(ins_type.length >0) {
      const result = await api_access('insert', 'func_data_sb3', ins_type) ;
      if(result.includes('fail')) {
        alert('proses data error');
        return;
      } else {
        const result = await api_access('insert', 'func_log_sb3', ins_type) ;
      }
    }
...
//----------------------------------------------
  if(event.target.getAttribute('data-method') === 'loc_submit') {
    ...
    const cek = await api_access('fetch', 'loc_list_sb3', {name: ins_mstr[0]['name']});
    if(cek.length === 0 || cek === undefined ){
      const result = await api_access('insert','loc_list_sb3',ins_mstr);
      if(!result.includes('fail')) {
...
//----------------------------------------------
  if(event.target.getAttribute('data-method') === 'spk_submit') {
    ...
    const result = await api_access('insert','func_data_sb3',ins_mstr);
    if(!result.includes('fail')) {
      const result2 = await api_access('insert','func_log_sb3',ins_mstr);
      alert('data berhasil ditambahkan');
      location.reload();
    } else {
