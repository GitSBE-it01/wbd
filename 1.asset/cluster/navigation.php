...
function nav($navArray) {
    ...
    */
    ...
    $dflt_list_style = 'w-[60vw] h-full flex flex-row';
        if(isset($navArray['list_style']) && $navArray['list_style'] !=='') {$dflt_list_style = $navArray['list_style'];}

    $home = '
    <div class="list-none w-[10vw] h-full hover:pt-2 hover:bg-slate-700 duration-200 pt-2 pl-10 pr-10 ease-in-out hover:border-b-4 hover:border-teal-300">
    ...
    $dflt_title_style = 'text-2xl underline h-full w-full pt-2 capitalize font-bold italic text-slate-200 text-right mr-[2vw]';
        if(isset($navArray['title_style']) && $navArray['title_style'] !=='') {$dflt_title_style = $navArray['title_style'];}
    $title = '
        <div class="w-[30vw] h-full flex">
            <h1 class="'
            .$dflt_title_style . '">'
            .$navArray['title']
            .'</h1>
