...
function connectToDatabaseNew() {
    // Load .env file manually
    $envConfig = parse_ini_file(ENV_FILE);

    // Access the environment variables
    $dbHost = $envConfig['db_host2'];
    $dbUsername = $envConfig['db_username2'];
    $dbPassword = $envConfig['db_password2'];
    $dbDatabase = $envConfig['db_name2'];
...
const otb = await ng_daily.fetchDataFilter({op_tran_date:'2024-07-08'}); // labor
const trans = await jig_trans.fetchDataFilter({start_date:'2024-07-08'}); 
const trans2 = await jig_trans.fetchDataFilter({end_date:'2024-07-08'}); 
